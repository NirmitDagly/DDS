...
class Helper {
    ...
    class func loadingSpinner(isLoading: Bool, isUserInteractionEnabled: Bool, withMessage message: String) {
        ...
        let window = keyWindow //UIApplication.shared.windows.filter {$0.isKeyWindow}.first
        var loader: FillableLoader = FillableLoader()
        let spinner = NVActivityIndicatorView(frame: .zero, type: .circleStrokeSpin, color: UIColor(Color.qikiColor), padding: 0)
        ...
        messageLabel.font = UIFont.init(name: "AvenirNext-Medium", size: 18)
